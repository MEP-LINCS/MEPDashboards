---
title: "HMEC240L Lineage"
output: 
  flexdashboard::flex_dashboard:
    orientation: columns
    source_code: embed
runtime: shiny
---

```{r global, include=FALSE}
# load data in 'global' chunk so it can be shared by all users of the dashboard
library(readr)
library(ggplot2)
library(plotly)
library(reshape2)
library(d3heatmap)

dfZoom <- function(x, min=.02, max=1){
  minMax <- quantile(unlist(x), probs=c(min,max), na.rm=TRUE)
  cl <- t(apply(x,1, function(c){
    c[c<minMax[1]] <- minMax[1]
    c[c>minMax[2]] <- minMax[2]
    return(c)
  }))
  return(data.frame(cl))
}
df <- read_tsv("./Data/HMEC240L_SSC_Level4.tsv")
df <- df[grep("Cytoplasm_PA_Gated_KRT19PositiveProportionNorm|EdUPositiveProportionNorm$|Spot_PA_SpotCellCount$|MEP|ECMp$|^Ligand$",colnames(df), value=TRUE)]
hmcols<-colorRampPalette(c("blue","white","red"))(16)

```


```{r}
# Reactive that returns the whole dataset if there is no brush
selectedData <- reactive({
  data <- brushedPoints(df, input$hist_brush, yvar="Cytoplasm_PA_Gated_KRT19PositiveProportionNorm",allRows = TRUE)
  if(!input$include) data <- data[!data$selected_,]
  else   data <- data[data$selected_,]
  if (nrow(data) == 0)
    data <- df
  write.table(data$MEP,"SelectedMEPs.csv",quote = FALSE, col.names = FALSE,row.names = FALSE)
  data
})
```

Column {data-width=500}
-----------------------------------------------------------------------

###  {data-width=450}


```{r, fig.height=1}
checkboxInput("include", "Include Selected", value = TRUE)
```


### KRT19 High Histogram {data-width=450}

```{r}

plotOutput("hist", brush = brushOpts(id = "hist_brush"))
output$hist <- renderPlot({
  ggplot(df, aes(Cytoplasm_PA_Gated_KRT19PositiveProportionNorm)) + geom_histogram(binwidth = .001)+
    labs(x="KRT19 High Proportion")+
   theme(axis.text.x = element_text(angle = 0, vjust = 0.5, hjust=1, size=rel(.8)),
          axis.title.x = element_text(size=rel(1)),
          plot.title = element_text(size = rel(.8)),
          strip.text = element_text(size = rel(.5)),
          legend.text=element_text(size = rel(.4)),legend.title=element_text(size = rel(.3)))
})
```


### KRT19 High and Ligands {data-width=450}

```{r}
renderPlot({
  ggplot(selectedData(), aes(factor(Ligand), Cytoplasm_PA_Gated_KRT19PositiveProportionNorm))  + geom_boxplot()+
      labs(x="Ligand", y="KRT19 High\nProportion")+
   theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, size=rel(.8)),
          axis.title.x = element_text(size=rel(1)),
          plot.title = element_text(size = rel(.8)),
          strip.text = element_text(size = rel(.5)),
          legend.text=element_text(size = rel(.4)),legend.title=element_text(size = rel(.3)))

})
```



### KRT19 High and ECM Proteins {data-width=450}

```{r}
renderPlot({
  ggplot(selectedData(), aes(factor(ECMp), Cytoplasm_PA_Gated_KRT19PositiveProportionNorm))  + geom_boxplot()+
      labs(x="ECM Protein", y="KRT19 High\nProportion")+
   theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1, size=rel(.8)),
          axis.title.x = element_text(size=rel(1)),
          plot.title = element_text(size = rel(.8)),
          strip.text = element_text(size = rel(.5)),
          legend.text=element_text(size = rel(.4)),legend.title=element_text(size = rel(.3)))

})
```


Column {data-width=500}
-----------------------------------------------------------------------


### KRT19 High vs. Spot Cell Count {data-width=450}

```{r}
renderPlotly({
  ggplot(selectedData(), aes(Spot_PA_SpotCellCount, Cytoplasm_PA_Gated_KRT19PositiveProportionNorm, text=MEP))  + geom_point()+
    labs(x="Cells per Spot", y="KRT19 High")+
   theme(axis.text.x = element_text(angle = 0, vjust = 0.5, hjust=1, size=rel(.8)),
          axis.title.x = element_text(size=rel(.8)),
          axis.title.y = element_text(size=rel(.7)),
          plot.title = element_text(size = rel(.8)),
          strip.text = element_text(size = rel(.5)),
          legend.text=element_text(size = rel(.4)),legend.title=element_text(size = rel(.3)))
})
```


### KRT19 High Heat Map {data-width=450}

```{r Heatmap}

renderD3heatmap({
#Cast to get ligands into columns
dfc <- dcast(selectedData()[c("ECMp","Ligand","Cytoplasm_PA_Gated_KRT19PositiveProportionNorm")],ECMp~Ligand, value.var = "Cytoplasm_PA_Gated_KRT19PositiveProportionNorm")

rownames(dfc) <- dfc$ECMp
dfc <- dfc[,!grepl("ECMp",names(dfc))]
d3heatmap(dfZoom(dfc, .05, .95), xaxis_font_size="5pt", colors=hmcols, yaxis_font_size="4pt",Rowv = FALSE, Colv=FALSE)

})

```


### Selected MEPs {data-width=450}

```{r SelectedMEPs}

  renderText({
selectedData()$MEP
})

```
